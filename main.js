...
const elem2 = (selector) => document.querySelectorAll(selector);
let users = [];
let row;
...
elem('.btn-save-edit').addEventListener('click', () => {
    saveUser();
    clearInputs();
    render();
});
elem('#users-list').addEventListener('click', () => {
...
elem('.btn-add').addEventListener('click', () => {
    addUser();
    clearInputs();
    render();
});
